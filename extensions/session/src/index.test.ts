...
describe("session extension", () => {
  ...
  it("tracks session message timestamps and skips elapsed reminder without prior assistant metadata", async () => {
    ...
    expect(stored?.metadata?.lastAssistantMessagePreview).toBe("fresh answer");
  ...
  it("injects elapsed time reminder when prior assistant output is stale", async () => {
    ...
    upsertSession({
      ...
      metadata: {
        ...
        lastAssistantMessagePreview: "Previous response",
      },
    });
    ...
    expect(capturedContent as string).toContain(
      "Last assistant message preview: Previous response",
    );
