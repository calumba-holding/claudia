...
const ASSISTANT_PREVIEW_LIMIT = 1_000;
...
export function truncateAssistantPreview(text: string): string {
  const trimmed = text.trim();
  if (trimmed.length <= ASSISTANT_PREVIEW_LIMIT) return trimmed;
  return `${trimmed.slice(0, ASSISTANT_PREVIEW_LIMIT).trimEnd()}...`;
}
...
export function buildElapsedTimeReminder(params: {
  ...
  const preview =
    typeof metadata.lastAssistantMessagePreview === "string"
      ? truncateAssistantPreview(metadata.lastAssistantMessagePreview)
      : "";
  ...
  if (preview) {
    lines.push(`Last assistant message preview: ${preview}`);
  }
